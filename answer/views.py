...
def answers(request):
    ...
    choices = ['A', 'B', 'C', 'D']
    return render(request, 'index/answers.html', {'answers' : answers, \
                                                  'questions' : questions, \
                                                  'choices' : choices}
                                                  )
...
def show(request):
    if request.is_ajax():
        id = request.GET.get('answerID')
        data = model_to_dict(Answer.objects.get(id=id))
    return HttpResponse(json.dumps(data), content_type='application/json')

def update(request):
    id = request.POST.get('id')
    
    data = Answer.objects.get(id = id)
